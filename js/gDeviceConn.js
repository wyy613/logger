...
function initlist () {
    return new Promise((resolve, reject) => {
        ...
        Ajax.get(`/search_device?addr=${searobj.addr}`, function (c) {
            getbak = JSON.parse(c)
            console.log(getbak)
            try {
...
function startSearch () {
    return new Promise((resolve, reject) => {
        ...
        getDataFromAjax('loggerpara_face_post').then((c) => {
            ...
            Ajax.get(`/scan_device`, function (q) {
                getarr = JSON.parse(q)
                function startTimer () {
                    // 每隔1秒刷新参数数据
                    setTimeout(function () {
                        ...
                        if (getarr.dev.length != 0 && getarr.state == 1) {
                            ...
                            Ajax.get(`/scan_device`, function (w) {
                                getarr = JSON.parse(w)
                                lis.dev = []
                                // 继续定时器
                                if (counter <= c.sumdevnum) {
...
oadd.addEventListener('click', function () {
    if (oadd.style.cursor != 'not-allowed') {
        ...
        Ajax.post(`/dev_addrset`, JSON.stringify(obj), function (c) {
            c = JSON.parse(c)
            console.log(c)
            if (c.state == 1) {
            ...
            i.addEventListener('click', function () {
                ...
                ensureEditButt.addEventListener('click', function () {
                    ...
                    if (ensureEditButt.style.cursor = 'not-allowed') {
                        Ajax.post(`/edit_device`, JSON.stringify(obj), function (c) {
                            c = JSON.parse(c)
                            ensureEditButt.style.cursor = 'pointer'
                            console.log(c)
                            if (c.state == 1) {
    ...
    return new Promise((res) => {
        ...
        ensureDeleteButt.addEventListener('click', function () {
            ...
            if (ensureDeleteButt.style.cursor = 'not-allowed') {
                Ajax.post(`/delete_device`, JSON.stringify(obj), function (c) {
                    c = JSON.parse(c)
                    console.log(c)
                    if (c.state == 1) {
                        showMessage(getPrompt("setSucc"), 2000, 'succContent', '&#xe616;')
...
bdelete.addEventListener('click', function () {
    ...
    if (connarr.length != 0) {
        ...
        if (postarr.dev.length != 0) {
            ...
            Ajax.post(`/delete_device`, JSON.stringify(postarr), function (c) {
                c = JSON.parse(c)
                if (c.state == 1) {
                    showMessage(getPrompt("setSucc"), 2000, 'succContent', '&#xe616;')
                    initlist().then((res) => {
...
nextpage.addEventListener('click', function () {
    ...
    parentDoc.querySelectorAll('.nav2li')[3].click()
